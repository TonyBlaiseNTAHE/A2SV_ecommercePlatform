...
describe('Orders endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    admin.role = 'Admin';
    await admin.save();
    const loginAdmin = await request(app).post('/auth/login').send({ email: 'admin2@example.com', password: 'AdminP@ss1!' });
    const adminToken = loginAdmin.body.object.token;

    const prodRes = await request(app)
      .post('/products')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ name: 'Orderable', description: 'For orders', price: 5.0, stock: 10, category: 'test' });
